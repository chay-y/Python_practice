...
load_wb = load_workbook('수료증\수료증명단.xlsx')
...
for i in range(len(name_list)):
    doc = docx.Document('수료증\수료증양식.docx')
    ...
    run=para.add_run('\n\n')
    run=para.add_run(' 제 '+ho_list[i]+' 1호\n')
    run.font.name='나눔고딕'
    ...
    doc.save(r'수료증/'+name_list[i]+".docx")
    convert(r'수료증/'+name_list[i]+".docx",
            r"수료증/"+name_list[i]+".pdf")